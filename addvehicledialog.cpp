...
void AddVehicleDialog::setupUI() {
    ...
    // 第0页：Bus
    {
        ...
        m_maintenanceSpin->setValue(2000);  // 默认值（卡车档次，中等）
        pf->addRow(QStringLiteral("维护费(元/月)："), m_maintenanceSpin);
    ...
    // 第1页：Car
    {
        ...
        m_maintenanceSpin->setDecimals(2);
        m_maintenanceSpin->setSuffix(QStringLiteral(" 元/月"));
        m_maintenanceSpin->setValue(1000);  // 默认值（卡车档次，中等）
        pf->addRow(QStringLiteral("维护费(元/月)："), m_maintenanceSpin);
        m_stacked->addWidget(page);
    }
    // 第2页：Truck
    {
        ...
        m_maintenanceSpin->setDecimals(2);
        m_maintenanceSpin->setSuffix(QStringLiteral(" 元/月"));
        m_maintenanceSpin->setValue(1500);  // 默认值（卡车档次，中等）
        pf->addRow(QStringLiteral("维护费(元/月)："), m_maintenanceSpin);
        m_stacked->addWidget(page);
    }
